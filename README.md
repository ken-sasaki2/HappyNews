...
#### ②XMLファイルの要素の順番と感情分析後の配列の要素順を合わせるアルゴリズムの構築
ニュースの元であるXMLファイルの要素をWatson APIに投げた後に、受け取るレスポンスの配列の要素順にずれが生じる不具合が発生し、感情分析結果で取得したニュースではないニュースを表示してしまう問題が発生しました。  
NewsViewControllerの' loadNewsDataメソッド '、並びにLanguageTranslatorModelの' sortTheTranslationResultsメソッド '、にてfor文を2種類扱うことで整合性を合わせることに成功しております。

## 今後の課題
#### テストコードの記述
今後の学習予定として、まずテストコードの学習をおこないます。  
HappyNewsを公開した時点では、XCTestの難易度が高く、テストコードは記述しておりません。  

学習法として、①XCTestの基礎知識を書籍『iOSアプリ開発自動テストの教科書〜』を使って身につけるという方法と、②MagicPodという自動テストをAIでおこなうツールを用いてアプリのテストを実施するという方法を検討中であります。

MagicPodは、note株式会社様や株式会社Gunosy様でも実用した事例が存在するツールなので、比較的モダンであり、次回開発するアプリには必ず実施する予定です。
